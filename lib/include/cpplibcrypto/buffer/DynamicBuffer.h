#ifndef CPPLIBCRYPTO_BUFFER_DYNAMICBUFFER_H_
#define CPPLIBCRYPTO_BUFFER_DYNAMICBUFFER_H_
...
public:
    ...
    explicit DynamicBuffer(const Size size)
        : DynamicBuffer() {
        resize(size);
    }

    /// Constructs the buffer with \ref count copies of \ref value value
    explicit DynamicBuffer(const Size count, const ValueType& value)
        : DynamicBuffer() {
        insert(end(), value, count);
    }

    /// Constructs the buffer with the content of the initializer list
    DynamicBuffer(std::initializer_list<ValueType> list)
        : DynamicBuffer() {
    ...
    ///
    /// Sensitive flag means that on destruction (which also includes reallocating the buffer) the data will
    /// not only be destroyed but also memset with a random bytes.
    ...
    Iterator erase(const Size from, const Size count = 1) {
    ...
    /// will be inserted. \param newSize The requested new size
    ...
    ///
    /// The element will be constructed using placement new expression. The arguments are forwarded using
    /// std::forward. \returns Reference to the inserted element
